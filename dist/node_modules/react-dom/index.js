(function (global, factory) {
	...
	const HostRoot = 3; //  react根节点
	const HostCompoment = 5; // <div>
	const HostText = 6; // 123

	class FiberNode {
	    tag; // 节点类型 () -> {}  -> FunctionComponent
	    pendingProps; // 最新的props
	    key;
	    type; // 当前节点的组件类型(函数组件，类组件，标签名)
	    stateNode; // DOM节点或者是组件实例
	    ref; // 引用，如(ref,React.createRef)
	    return; // 父fiber
	    sibling; // 兄弟fiber
	    child; // 第一个子fiber节点
	    index; // 当前节点在其兄弟节点中的位置
	    memoizedProps; // 上一次渲染使用的props
	    memoizedState; // 状态
	    alternate; // 指向上次渲染的fiber(current和workInProgress的互指)
	    flags; // fiber需要执行的操作
	    subtreeFlags; // 当前节点的子树的副作用标识
	    updateQueue; // 存放state更新队列(如setState的调用)
	...
	class FiberRootNode {
	    container; // 宿主环境的容器，比如 DOM 中的 #root
	    current; // 指向当前渲染中的根 FiberNode（HostRootFiber）
	    finishedWork; // 调度完成、等待 commit 的 Fiber 树
